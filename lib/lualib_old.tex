...
--       local string = "\\draw[] ("..x.." ,"..originY.." ) arc (0:"..angle..":1.5) -- node[above,sloped] { $r_{d}$} ++("..angle..":"..lenght..") --++("..angle..":1) coordinate (temp);"
...
--       local string = "\\draw[] ("..x.." ,"..originY.." ) arc (0:"..angle..":1.5) -- node[above,sloped] { $r_{"..i.."}$} ++("..angle..":"..lenght..")  --++("..angle..":1) coordinate (temp);"
--       local string2 = "\\draw[fill=white] (temp) circle ("..radius.."-0.1) coordinate (t2);"
...
--       tikzDrawAlongCircle(4, originX, originY+1, 2, fir_angle, las_angle)
--       end
--     end

function tikzDrawHOSVD(max_branch, originX, originY, angle, radius)
      -- label the core
      local string = "\\draw[] node at ("..originX.."-1.5 ,"..originY.." ) {$G$};"
      tex.print(string)
      for i = 1,max_branch,1 do
      local x = originX
      local lenght = 3
      ...
      if (i == max_branch) then
      local string = "\\draw[] ("..x.." ,"..originY.." ) arc (0:"..angle..":1.5) -- node[above,sloped] { $r_{d}$} ++("..angle..":"..lenght..") --++("..angle..":1) coordinate (temp);"
      local string2 = "\\draw[fill=white] (temp) circle ("..radius..")  coordinate (t2);"
      local string3 = "\\draw[] (t2) node {$U^{(d)}$} ++("..angle..":"..radius..") -- ++("..angle..":1) ++("..angle..":0.5) node {$n_{d}$};"
      tex.print(string..string2..string3)
      -- Create the other node
      elseif (i <= max_branch - 2) then
      local string = "\\draw[] ("..x.." ,"..originY.." ) arc (0:"..angle..":1.5) -- node[above,sloped] { $r_{"..i.."}$} ++("..angle..":"..lenght..")  --++("..angle..":1) coordinate (temp);"
      local string2 = "\\draw[fill=white] (temp) circle ("..radius..") coordinate (t2);"
      local string3 = "\\draw[] (t2) node {$U^{("..i..")}$} ++("..angle..":"..radius..") -- ++("..angle..":1) ++("..angle..":0.5) node {$n_{"..i.."}$};"
      tex.print(string..string2..string3)
      end
      -- Create the dot between last and other nodes
      target_branch = 1
      dotradius = 3
      margin = 360/max_branch * 0.4
      fir_angle = 360/max_branch * (target_branch-1) + margin
      las_angle = 360/max_branch * (target_branch+1) - margin
      tikzDrawAlongCircle(5, originX-1.5, originY, dotradius, radius*5.4, fir_angle, las_angle)
      end
    end
...
function tikzDrawAlongCircle(nb_dots, origin_x, origin_y, dotradius, radius, angle1, angle2)
    local function draw_dot_at_angle(angle)
      ...
      tex.sprint(string.format("\\filldraw[black] (%f,%f) circle (%f pt);", x, y, dotradius))
    end
    ...
    function tikztensorbranchCustom(max_branch, origin_x, origin_y, radius, d_branch)
      ...
      for i = 1,max_branch,1 do
          ...
          elseif (i == max_branch and max_branch > 3 and d_branch)  then
            ...
            fir_angle = 360/max_branch * (target_branch-1) + margin
            las_angle = 360/max_branch * (target_branch+1) - margin          
            tikzDrawAlongCircle(4, origin_x, origin_y, 2, 3.5, fir_angle, las_angle)
          elseif (i == max_branch and d_branch)  then
            ...
            tikzDrawAlongCircle(3, origin_x, origin_y, 3.5, 3.5, fir_angle, las_angle)
          else
    ...
    --         local lbranch = "\\draw[] ("..x..","..origin_y..") ++("..branch_lenght.."/2,"..height_rank..") node {$r_{d-1}$};"
    ...
    --       local string4 = "\\draw[] ("..x+radius..","..origin_y..") ++("..branch_lenght.."/2,"..height_rank..") node { $r_{"..i.."}$};"
      ...
      for i = 1,max_branch,1 do
        if (i == max_branch) then
          if (d_branch) then
            ...
            local lbranch = "\\draw[] ("..x..","..origin_y..") ++("..branch_lenght.."/2,"..height_rank..") node { $r_{d-1}$};"
            x = x + branch_lenght + radius
            local string = "\\draw[] ("..x..","..origin_y..") node {$G^{(d)}$} circle ("..radius..");"
            local string2 = "\\draw[] ("..x..","..origin_y+radius..") -- ++(0,"..branch_lenght..") ++(0,0.5) node {$n_{d}$};"
            tex.print(lbranch..string3..string..string2)
          else
            local string = "\\draw[] ("..x..","..origin_y..") node {$G^{("..i..")}$} circle ("..radius..");"
            local string2 = "\\draw[] ("..x..","..origin_y+radius..") -- ++(0,"..branch_lenght..") ++(0,0.5) node {$n_{"..i.."}$};"
            tex.print(string..string2)
          end
        else
